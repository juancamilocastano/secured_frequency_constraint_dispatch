...
function process_parameters!(m::Model, data::Dict)
   ...
   for i in ID_Pump
      ...
      m.ext[:parameters][:PGminD][i] =  d[SubString(i,1:length(i)-2)]["minStableOperatingPoint"]
...
Ppc= m.ext[:variables][:Ppc] = @variable(m, [i=ID_Pump,j=J],lower_bound=0,base_name="Ppc") #Charging variable

Ppd= m.ext[:variables][:Ppd] = @variable(m, [i=ID_Pump,j=J],lower_bound=0,base_name="Ppd") #Discharging variable
...
zp= m.ext[:variables][:zp] = @variable(m, [i=ID_Pump,j=J], binary=true, base_name="zp") #Charging variable
...
con2_5_1=m.ext[:constraints][:con2_5] = @constraint(m, [i=ID_Nuclear,j=J],
g[i,j]==0.98*GmaxD[i]*zuc[i,j]
)
...
#Constraints frequency nadir as rotate second order cone
 ...
 con14_1_Pump=m.ext[:constraints][:con14_1_Pump] = @constraint(m,[i=ID_Pump,j=J],yp[i,j] ==2*deltaf*(sum(Inertia_Expression[:,j])-Inertia_Expression[i,j])/FO-sum(re[:, j])*Dte/2-sum(rb[:, j])*Dtb/2)
   con14_2_Pump=m.ext[:constraints][:con14_2_Pump] = @constraint(m,[i=ID_Pump,j=J],zp[i,j] ==(sum(rg[:, j]))/Dtg)
 ...
 con16=m.ext[:constraints][:con16] = @constraint(m, [i=ID,j=J],pl[j].<=0.000001 + sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j])+sum(rgp[:,j])-rg[i, j]))
   ...
   con15_pump= m.ext[:constraints][:con15_pump] = @constraint(m, [i=ID_Pump,j=J],pl[j].>=sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j]))*Dtb/Dtg)
   con16_pump=m.ext[:constraints][:con16_pump] = @constraint(m, [i=ID_Pump,j=J],pl[j].<=0.000001 + sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j])))
...
results = Dict("g" => g,
"rg" => rg, "re" => re, "rb" => rb, "pl" => pl, "pbc" => pbc, "pbd" => pbd,
               "eb" => eb, "pe" => pe, "hfe" => hfe, "pe_c"=>pe_c, "hfgdinyec" => hfgdinyec,
               ...
               ,"costs_rg" => sum(value.(rg_costs)), "costs_rb" => sum(value.(rb_costs)),
               "costs_re" => sum(value.(re_costs)), "costs_scu_UC" => sum(value.(scu_UC)),
               "costs_scu_UC_e" => sum(value.(scu_UC_e)), "costs_h" => sum(value.(h_costs)),
               "costs_total_reserve" => sum(value.(rb_costs)) + sum(value.(re_costs)) + sum(value.(rg_costs)),
