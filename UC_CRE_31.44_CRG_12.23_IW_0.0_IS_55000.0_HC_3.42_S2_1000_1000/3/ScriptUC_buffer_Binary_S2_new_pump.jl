...
Ppc= m.ext[:variables][:Ppc] = @variable(m, [i=ID_Pump,j=J],lower_bound=minStableOperatingPointC[i],base_name="Ppc") #Charging variable

Ppd= m.ext[:variables][:Ppd] = @variable(m, [i=ID_Pump,j=J],lower_bound=minStableOperatingPointD[i],base_name="Ppd") #Discharging variable
...
zpvector= [zp[i,j] for i in ID_Pump, j in J]
...
"rg" => rg,"rgp" => rgp, "re" => re, "rb" => rb, "pl" => pl, "pbc" => pbc, "pbd" => pbd,
               ...
               ,"zp"=>zp,"costs_rg" => sum(value.(rg_costs)), "costs_rb" => sum(value.(rb_costs)),
