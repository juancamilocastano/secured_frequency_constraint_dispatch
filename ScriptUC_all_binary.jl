...
upramrate=m.ext[:parameters][:upramprate]
upramrate=Dict(key => value / Pbase for (key, value) in upramprate)
...
zb = m.ext[:variables][:zb] = @variable(m, [i=ID_BESS,j=J], binary=true, base_name="zb") #Zb
yb= m.ext[:variables][:yb] = @variable(m, [i=ID_BESS,j=J], binary=true, base_name="yb") #Yb
...
pbc = m.ext[:variables][:pbc] = @variable(m, [i=ID_BESS,j=J],upper_bound=PBmax[i], base_name="pbc") #Charging power of the batteries
pbd = m.ext[:variables][:pbd] = @variable(m, [i=ID_BESS,j=J],upper_bound=PBmax[i], base_name="pbd") #Discharging power of the batteries
...
hfgc = m.ext[:variables][:hfgc] = @variable(m, [i=ID_E,j=J],upper_bound= Max_h_f[i],base_name="hfgc") #Hydrogen flow limit of the hydrogen flowing trhow the hydrogen pipeline
hfgd= m.ext[:variables][:hfgd] = @variable(m, [i=ID_E,j=J],upper_bound= Max_h_f[i],base_name="hfgd") #Hydrogen flow limit of the hydrogen flowing trhow the hydrogen pipeline
zge = m.ext[:variables][:zsge] = @variable(m, [i=ID_E,j=J], binary=true, base_name="zsge") #Zge
yge = m.ext[:variables][:yge] = @variable(m, [i=ID_E,j=J], binary=true, base_name="yge") #Yge
...
zse = m.ext[:variables][:zse] = @variable(m, [i=ID_E,j=J], binary=true, base_name="zse") #Zse
yse = m.ext[:variables][:yse] = @variable(m, [i=ID_E,j=J], binary=true, base_name="yse") #Yse
...
#Constraints upper and lower bound charging power of the batteries
con2_2_a=m.ext[:constraints][:con2_2_a] = @constraint(m, [i=ID_BESS,j=J],
pbc[i,j].<=PBmax[i]*zb[i,j]
)
con2_2_b=m.ext[:constraints][:con2_2_b] = @constraint(m, [i=ID_BESS,j=J],
pbd[i,j].<=PBmax[i]*yb[i,j]
)

#Constraint upper and lower bound charging and discharging flows hydrogen tank
con2_2_c=m.ext[:constraints][:con2_2_c] = @constraint(m, [i=ID_E,j=J],
hfsc[i,j].<=Max_h_f[i]*zge[i,j]
)
con2_2_d=m.ext[:constraints][:con2_2_d] = @constraint(m, [i=ID_E,j=J],
hfsd[i,j].<=Max_h_f[i]*yge[i,j]
)
#Constraint upper and lower bound charging and discharging flows pipeline
con2_2_e=m.ext[:constraints][:con2_2_e] = @constraint(m, [i=ID_E,j=J],
hfgc[i,j].<=Max_h_f[i]*zse[i,j]
)
con2_2_f=m.ext[:constraints][:con2_2_f] = @constraint(m, [i=ID_E,j=J],  
hfgc[i,j].<=Max_h_f[i]*zse[i,j]
)  

#on/off status constraintis
con2_2_g=m.ext[:constraints][:con2_2_g] = @constraint(m, [i=ID_BESS,j=J],
zb[i,j]+yb[i,j].<=1
)
con2_2_h=m.ext[:constraints][:con2_2_h] = @constraint(m, [i=ID_E,j=J],
zge[i,j]+yge[i,j].<=1
)
con2_2_i=m.ext[:constraints][:con2_2_i] = @constraint(m, [i=ID_E,j=J],
zse[i,j]+yse[i,j].<=1
)
...
g[i,j] +rg[i,j]- g[i,j-1] <= upramprate[i] * zuc[i,j] + (GminD[i] - upramprate[i]) * v[i,j]
)
con2_2_b = m.ext[:constraints][:con2_2_b] = @constraint(m, [i=ID,j=J[2:end]],
g[i,j-1] - g[i,j] <= downramprate[i] * z[i,j] + GminD[i] * w[i,j]
)
=#
...
#On status constraint for nuclear
con2_5_1=m.ext[:constraints][:con2_5_1] = @constraint(m, [i=ID_Nuclear,j=J],
rg[i,j]==0
)
...
con10=m.ext[:constraints][:con10] = @constraint(m, [i=ID_E,j=J],hfe[i,j]+hfsd[i,j]+hfgd[i,j]==Eload_factor[i]*PEmax[i]/(Eeff[i])+hfsc[i,j]+hfgc[i,j])
...
con13_8=m.ext[:constraints][:con13_8] = @constraint(m, [i=ID_E,j=J],pe_c[i,j]==compresor_power[i]*(hfe[i,j]+hfsc[i,j]+hfsd[i,j]+hfgd[i,j]+hfgc[i,j]))
...
p_h_storage = plot(hfscvec[1,:], 
     ...
     label = "hfscvec",
     ylims = (0, maximum([maximum(hfscvec[1,:]), maximum(hssvec[1,:]), maximum(hfsdvec[1,:])]) + 10))  # Added ylims
...
#Constraint ramp up and ramp down limits
con2_2_a = m.ext[:constraints][:con3b] = @constraint(m, [i=ID_GR,j=J[2:end]],
g[i,j] +rg[i,j]- g[i,j-1] <= upramprate[i] * zuc[i,j] + (GminD[i] - upramprate[i]) * v[i,j]
)
con2_2_b = m.ext[:constraints][:con3c] = @constraint(m, [i=ID_GR,j=J[2:end]],
g[i,j-1] - g[i,j] <= downramprate[i] * z[i,j] + GminD[i] * w[i,j]
)

con2_2_an = m.ext[:constraints][:con3b] = @constraint(m, [i=ID_Nuclear,j=J[2:end]],
g[i,j] +rg[i,j]- g[i,j-1] <= 0 * zuc[i,j] + (GminD[i] - upramprate[i]) * v[i,j]
)
con2_2_bn = m.ext[:constraints][:con3c] = @constraint(m, [i=ID_Nuclear,j=J[2:end]],
g[i,j-1] - g[i,j] <= 0 * z[i,j] + GminD[i] * w[i,j]
)
