...
WC[j]*Installed_W + SC[j]*Installed_S-RCU[i] + sum(g[i,j] for i in ID) - sum(pbc[i,j] for i in ID_BESS) + sum(pbd[i,j] for i in ID_BESS) == D[j] + sum(pe[i,j] for i in ID_E) + sum(pe_c[i,j] for i in ID_E)
)
