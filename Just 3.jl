...
Ppd= m.ext[:variables][:Ppd] = @variable(m, [i=ID_Pump,j=J],lower_bound=0,upper_bound=PGmaxD[i],lower_bound=minStableOperatingPointD[i],base_name="Ppd") #Discharging variable
...
for j in J
   Sum_Inertia_Vector[j]=sum(Inertia_Vector[i]*value.(zuc[i,j]) for i in ID)+ PInertia_Vector["Pump_1"]*value.(zpcommit[1,j])
end
