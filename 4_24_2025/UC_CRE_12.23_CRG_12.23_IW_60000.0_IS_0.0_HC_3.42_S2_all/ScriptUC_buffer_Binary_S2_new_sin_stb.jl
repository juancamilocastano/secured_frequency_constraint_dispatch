...
function process_time_series_data!(m::Model, ts::DataFrame)
   number_hours=nrow(ts)
    ...
    m.ext[:timeseries][:WC] =  capacity_factor_renewable.Wind_Capacity_Factor_2[1:number_hours]
...
#Constraignt upper bound reserve
...
#Constraint lower bound generators power
...
"rg" => rg, "re" => re, "rb" => rb, "pl" => pl, "pbc" => pbc, "pbd" => pbd,
               ...
               "ze" => ze, 
               "zb" => zb, "costs_g" => sum(value.(g_costs))
