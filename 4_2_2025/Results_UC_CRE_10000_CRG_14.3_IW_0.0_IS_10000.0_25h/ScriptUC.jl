...
hfgc = m.ext[:variables][:hfgc] = @variable(m, [i=ID_E,j=J],lower_bound=0,upper_bound= Max_h_f[i],base_name="hfgc") #Hydrogen flow limit of the hydrogen flowing trhow the hydrogen pipeline
hfgd= m.ext[:variables][:hfgd] = @variable(m, [i=ID_E,j=J],lower_bound=0,upper_bound= Max_h_f[i],base_name="hfgd") #Hydrogen flow limit of the hydrogen flowing trhow the hydrogen pipeline
...
WC[j]*Installed_W+sum(g[i,j] for i in ID) -sum(pbc[i,j] for i in ID_BESS)+sum(pbd[i,j] for i in ID_BESS) -RCU[j]== D[j]-SC[j]*Installed_S+sum(pe[i,j] for i in ID_E)+sum(pe_c[i,j] for i in ID_E)
)
