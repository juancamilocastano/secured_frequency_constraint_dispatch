...
for j in J
   ...
   con14_1=m.ext[:constraints][:con14_1] = @constraint(m,[i=ID,j=J],y[i,j] ==2*deltaf*(sum(values(inertia_Constant))-inertia_Constant[i])/FO-sum(re[:, j])*Dte/2)
...
for j in J
 ...
 con14_1=m.ext[:constraints][:con14_1] = @constraint(m,[i=ID,j=J],y[i,j] ==2*deltaf*(sum(values(inertia_Constant))-inertia_Constant[i])/FO-sum(re[:, j])*Dte/2-sum(rb[:, j])*Dtb/2)
