...
zucvaluesfix=value.(m.ext[:variables][:zuc])
zevaluesfix=value.(m.ext[:variables][:ze])
zesuvaluesfix=value.(m.ext[:variables][:zesu])
zestbvaluesfix=value.(m.ext[:variables][:zestb])
zbvaluesfix=value.(m.ext[:variables][:zb])
zpcommitvaluesfix=value.(m.ext[:variables][:zpcommit])
   ...
   #https://www.sciencedirect.com/science/article/pii/S0378779624005649 according to this paper, the tolerant gap is between 0.1% and 0.01%
   set_optimizer_attribute(m2, "MIPGap", 0.00015) 
   #set_optimizer_attribute(m, "TimeLimit", 3600)  # 1 hour
   set_optimizer_attribute(m2, "Threads", 20)       # Use 8 threads
      
   m2.ext[:variables] = Dict()
   m2.ext[:expressions] = Dict()
   m2.ext[:constraints] = Dict()
   m2.ext[:objective] = Dict()


   zuc = m2.ext[:variables][:zuc] = @variable(m2, [i=ID,j=J],binary=true, base_name="commitment")
   v = m2.ext[:variables][:v] = @variable(m2, [i=ID,j=J], binary=true, base_name="start_up")
   w = m2.ext[:variables][:w] = @variable(m2, [i=ID,j=J], binary=true, base_name="shoot_down")
   g = m2.ext[:variables][:g] = @variable(m2, [i=ID,j=J],lower_bound=0, base_name="generation") #Power generation generators
   x = m2.ext[:variables][:x] = @variable(m2, [i=ID,j=J],lower_bound=0, base_name="x") #Auxiliary variable rotate second order cone
   y = m2.ext[:variables][:y] = @variable(m2, [i=ID,j=J],lower_bound=0, base_name="y") #Auxiliary variable rotate second order cone
   z = m2.ext[:variables][:z] = @variable(m2, [i=ID,j=J],lower_bound=0, base_name="z") #Auxiliary variable rotate second order cone
   xp = m2.ext[:variables][:xp] = @variable(m2, [i=ID_Pump,j=J],lower_bound=0, base_name="x") #Auxiliary variable rotate second order cone
   yp = m2.ext[:variables][:yp] = @variable(m2, [i=ID_Pump,j=J],lower_bound=0, base_name="y") #Auxiliary variable rotate second order cone
   ...
   pbd = m2.ext[:variables][:pbd] = @variable(m2, [i=ID_BESS,j=J],lower_bound=0, base_name="pbd") #Discharging power of the batteries
   eb = m2.ext[:variables][:eb] = @variable(m2, [i=ID_BESS,j=J], lower_bound= EBmax[i]*(1-DOD_max[i]), upper_bound=EBmax[i] , base_name="eb") #Energy bounds of the batteries
   zb = m2.ext[:variables][:zb] = @variable(m2, [i=ID_BESS,j=J], binary=true, base_name="on_off_b")

   hfe= m2.ext[:variables][:hfe] = @variable(m2, [i=ID_E,j=J],lower_bound=0, upper_bound= Max_h_f[i], base_name="hfe") #Hydrogen flow limit of the hydrogen produced by electrolyzers
   hfgdinyec = m2.ext[:variables][:hfgdinyec] = @variable(m2, [i=ID_E,j=J],lower_bound=0,upper_bound= Max_h_f[i],base_name="hfgdinyec") #Hydrogen flow limit of the hydrogen flowing trhow the hydrogen pipeline
   hfgdcon= m2.ext[:variables][:hfgdcon] = @variable(m2, [i=ID_E,j=J],lower_bound=0,upper_bound= Max_h_f[i],base_name="hfgdcon") #Hydrogen flow limit of the hydrogen flowing trhow the hydrogen pipeline
   #zhf = m2.ext[:variables][:zhf] = @variable(m2, [i=ID_E,j=J], binary=true, base_name="on_off_b")
   ze = m2.ext[:variables][:ze] = @variable(m2, [i=ID_E,j=J], binary=true, base_name="on_off_E")
   zesu = m2.ext[:variables][:zesu] = @variable(m2, [i=ID_E,j=J], binary=true, base_name="on_off_E_startup")
   #zestb= m2.ext[:variables][:zestb] = @variable(m2, [i=ID_E,j=J], binary=true, base_name="on_off_E_stand_by")
   zestb= m2.ext[:variables][:zestb] = @variable(m2, [i=ID_E,j=J], binary=true, base_name="on_off_E_stand_by")


   pe = m2.ext[:variables][:pe] = @variable(m2,  [i=ID_E,j=J],lower_bound= 0, upper_bound=PEmax[i], base_name="pe") #Power consumption electrolyzer
   pe_c= m2.ext[:variables][:pe_c] = @variable(m2,  [i=ID_E,j=J], base_name="pe_c") #Power consumption of compressor electrolyzer 
   hss = m2.ext[:variables][:hss] = @variable(m2, [i=ID_E,j=J],lower_bound=Min_h_s[i], upper_bound= Max_h_s[i], base_name="hss") #hydrogen storage limit


   #Pump variables

   Ppc= m2.ext[:variables][:Ppc] = @variable(m2, [i=ID_Pump,j=J],lower_bound=0,upper_bound=PGmaxD[i],base_name="Ppc") #Charging variable

   Ppd= m2.ext[:variables][:Ppd] = @variable(m2, [i=ID_Pump,j=J],lower_bound=0,upper_bound=PGmaxD[i],base_name="Ppd") #Discharging variable

   Pener= m2.ext[:variables][:Pener] = @variable(m2, [i=ID_Pump,j=J],lower_bound=PEBmax[i]*(1-PDOD_max[i]),upper_bound=PEBmax[i],base_name="Pener") #Energy variable

   zpcommit= m2.ext[:variables][:zpcommit] = @variable(m2, [i=ID_Pump,j=J], binary=true, base_name="zp") #Charging variable

   rgp= m2.ext[:variables][:rgp] = @variable(m2, [i=ID_Pump,j=J],lower_bound=0,upper_bound=PmaxFrdelivarable[i], base_name="rgp") #Reserve provided by generators


   #create affine expressions

   g_costs=m2.ext[:expressions][:g_costs] = @expression(m2, [i=ID,j=J],g[i,j]*CostFuel[i]*Pbase
   )
   rg_costs=m2.ext[:expressions][:rg_costs] = @expression(m2, [i=ID,j=J],rg[i,j]*res_cost_g[i]*Pbase
      )

   rgp_costs=m2.ext[:expressions][:prg_costs] = @expression(m2, [i=ID_Pump,j=J],rgp[i,j]*Pres_cost_g[i]*Pbase
      )
   rb_costs=m2.ext[:expressions][:rb_costs] = @expression(m2, [i=ID_BESS,j=J],rb[i,j]*res_cost_b[i]*Pbase
      )
   re_costs=m2.ext[:expressions][:re_costs] = @expression(m2, [i=ID_E,j=J],re[i,j]*res_cost_e[i]*Pbase
      )
   h_costs=m2.ext[:expressions][:h_costs] = @expression(m2, [i=ID_E,j=J],(hfgdcon[i,j]-hfgdinyec[i,j])*hydrogenCost[j]*Mbase
      )
   scu_UC=m2.ext[:expressions][:scu_UC] = @expression(m2, [i=ID,j=J], startupCost[i]*v[i,j])

   scu_UC_e=m2.ext[:expressions][:scu_UC_e] = @expression(m2, [i=ID_E,j=J], start_up_cost_e[i]*zesu[i,j])
   ...
   Installed_W_F=Installed_W*Pbase
   Installed_S_F=Installed_S*Pbase
   folder_name_plot="UC_CRE_$(RE_costs)_CRG_$(RG_costs)_IW_$(Installed_W_F)_IS_$(Installed_S_F)_S1_Interval_3_gap_0.00015_zuc_fix"
   ...
   Combined_ID = vcat(collect(ID), collect(ID_Pump)) 
   Inertia_Expression=m2.ext[:expressions][:Inertia_Expression] = @expression(m2, [i in Combined_ID, j in J],
      i in ID ? Inertia_Vector[i] * zuc[i,j] : PInertia_Vector[i]
   )


   # Inertia expression without considering pump
   #Inertia_Expression=m2.ext[:expressions][:Inertia_Expression] = @expression(m2, [i=ID,j=J],Inertia_Vector[i]*zuc[i,j])

   #Create the objective function
   obj= m2.ext[:objective] = @objective(m2,Min, (sum(g_costs)+sum(rg_costs)+sum(rgp_costs)+sum(rb_costs)+sum(re_costs)+sum(scu_UC)+sum(h_costs)+sum(scu_UC_e)) #Objective function
   )

   con1=m2.ext[:constraints][:con1] = @constraint(m2, [j=J],
   WC[j]*Installed_W + SC[j]*Installed_S-RCU[j] + sum(g[i,j] for i in ID) - sum(pbc[i,j] for i in ID_BESS) + sum(pbd[i,j] for i in ID_BESS)-sum(Ppc[i,j] for i in ID_Pump) +sum(Ppd[i,j] for i in ID_Pump) == D[j] + sum(pe[i,j] for i in ID_E) + sum(pe_c[i,j] for i in ID_E)
   )

   #constraint renewable curtailment
   con1_1=m2.ext[:constraints][:con1_1] = @constraint(m2, [j=J],
   RCU[j] <= WC[j]*Installed_W + SC[j]*Installed_S
   )
   ...
   #)
   #Constraignt upper bound generators power
   con2_1=m2.ext[:constraints][:con2_1] = @constraint(m2, [i=ID,j=J],
   g[i,j]+rg[i,j].<=GmaxD[i]*zuc[i,j]
   )
   #Constraint upper bound generators power
   con2_2=m2.ext[:constraints][:con2_2] = @constraint(m2, [i=ID,j=J],
   GminD[i]*zuc[i,j].<=g[i,j]+rg[i,j]
   )

   con2_1_2=m2.ext[:constraints][:con2_1_2] = @constraint(m2, [i=ID,j=J],
   g[i,j].<=GmaxD[i]*zuc[i,j]
   )
   #Constraint upper bound generators power
   con2_2_2=m2.ext[:constraints][:con2_2_2] = @constraint(m2, [i=ID,j=J],
   GminD[i]*zuc[i,j].<=g[i,j]
   )


   #Constraint upper bound reserve pump
   con1_2_Pump=m2.ext[:constraints][:con1_2_Pump] = @constraint(m2, [i=ID_Pump,j=J],
   rgp[i,j].<=PmaxFrdelivarable[i]*zpcommit[i,j]
   )
   #Constraint upper bound pump discharging mode
   con2_1_Pump=m2.ext[:constraints][:con2_1_Pump] = @constraint(m2, [i=ID_Pump,j=J],
   Ppd[i,j]+rgp[i,j].<=PGmaxD[i]*zpcommit[i,j]
   )


   #Constraint lower bound pump discharging mode
   con2_1_1_Pump=m2.ext[:constraints][:con2_1_1_Pump] = @constraint(m2, [i=ID_Pump,j=J],
   minStableOperatingPointD[i]*zpcommit[i,j].<=Ppd[i,j]+rgp[i,j]
   )


   #Constraint upper bound pump charging mode
   con2_2_Pump=m2.ext[:constraints][:con2_2_Pump] = @constraint(m2, [i=ID_Pump,j=J],
   Ppc[i,j].<=PGmaxD[i]*(1-zpcommit[i,j])
   )

   #constraint lower bound pump charging mode
   con2_2_1_Pump=m2.ext[:constraints][:con2_2_1_Pump] = @constraint(m2, [i=ID_Pump,j=J],
   Ppc[i,j].>=minStableOperatingPointC[i]*(1-zpcommit[i,j])
   )
   ...
   #Constraint charging and discharging mode of the batteries
   con2_1_1=m2.ext[:constraints][:con2_1_1] = @constraint(m2, [i=ID_BESS,j=J],
   pbc[i,j].<=PBmax[i]*(1-zb[i,j])
   )
   con2_1_2=m2.ext[:constraints][:con2_1_2] = @constraint(m2, [i=ID_BESS,j=J],
   pbd[i,j].<=PBmax[i]*zb[i,j]
   )


   con2_2_a = m2.ext[:constraints][:con2_2_a] = @constraint(m2, [i=ID,j=J[2:end]],
   g[i,j] +rg[i,j]- g[i,j-1] <= upramprate[i] ) # constraint taken from Mathematical Programming for Power Systems Operation

   #con2_2_a = m2.ext[:constraints][:con2_2_a] = @constraint(m2, [i=ID,j=J[2:end]],
   #g[i,j] - g[i,j-1] <= upramprate[i] ) # constraint taken from Mathematical Programming for Power Systems Operation

   con2_2_b = m2.ext[:constraints][:con2_2_b] = @constraint(m2, [i=ID,j=J[2:end]],
   g[i,j-1] - g[i,j] <= downramprate[i] )


   #Initial status generators (unit commitment)
   con2_3=m2.ext[:constraints][:con2_3] = @constraint(m2, [i=ID,j=J[1]],1-zuc[i,j]+v[i,j]-w[i,j]==0)

   #Status generators (unit commitment)
   con2_4=m2.ext[:constraints][:con2_4] = @constraint(m2, [i=ID,j=J[2:end]],
   zuc[i,j-1]-zuc[i,j]+v[i,j]-w[i,j]==0
   )

   con_2_4_1=m2.ext[:constraints][:con_2_4_1]=@constraint(m2, [i=ID,j=J],+v[i,j]+w[i,j]<=1
   )


   #On status constraint for nuclear
   con2_5=m2.ext[:constraints][:con2_5] = @constraint(m2, [i=ID_Nuclear,j=J],
   zuc[i,j]==1
   )
   ...
   #Minimum Up Time constraint
   con2_6 = m2.ext[:constraints][:con2_6] = Dict()
   for i in ID
      if MDT[i] > 0  # Only proceed if MUT[i] is positive, to avoid issues with MDT=0
         for j in J[MUT[i]:end]
            con2_6[i,j] =  @constraint(m2,
            zuc[i,j] >= sum(v[i,j-jj] for jj in 0:MUT[i]-1)
            )
         end
      end
   ...
   #Minimum Down Time constraint
   con2_7 = m2.ext[:constraints][:con2_7] = Dict()
   for i in ID
      if MDT[i] > 0  # Only proceed if MDT[i] is positive
         for j in J[MDT[i]:end]
            con2_7[i,j] = @constraint(m2,
                  1 - zuc[i,j] >= sum(w[i,j-jj] for jj in 0:MDT[i]-1)
            )
         end
      end
   ...
   con3=m2.ext[:constraints][:con3] = @constraint(m2, [i=ID,j=J],
   pl[j].>=g[i,j]
   )
   con3_Pump=m2.ext[:constraints][:con3] = @constraint(m2, [i=ID_Pump,j=J],
   pl[j].>=Ppd[i,j]
   )

   #= Constraints that include the loss of wind and solar generation
   con3_1=m2.ext[:constraints][:con3_1] = @constraint(m2, [i=ID,j=J],
   pl[j].>=SC[j]*Installed_S
   )

   con3_2=m2.ext[:constraints][:con3_1] = @constraint(m2, [i=ID,j=J],
   pl[j].>=WC[j]*Installed_W*0
   )
   =#

   #Constraint upper bound reserve provided by BESS
   con4=m2.ext[:constraints][:con4] = @constraint(m2, [i=ID_BESS,j=J],
   rb[i,j].<=PBmax[i]+pbc[i,j]-pbd[i,j]
   )

   #Constraint upper bound reserve provided by Electrolyzer
   con5=m2.ext[:constraints][:con5] = @constraint(m2, [i=ID_E,j=J],re[i,j].<=pe[i,j]-PEmin[i]*ze[i,j] 
   )



   #Constraint end energy value of the batteries
   con6=m2.ext[:constraints][:con6] = @constraint(m2, [i=ID_BESS,j=J[end]],End_e_b[i]-eb[i,j]==Beffc[i]*pbc[i,j]-pbd[i,j]/Beffd[i])
   #con6=m2.ext[:constraints][:con6] = @constraint(m2, [i=ID_BESS,j=J[end]],End_e_b[i]==eb[i,j])


   #Constraint initial value energy of the batteries
   #con7=m2.ext[:constraints][:con7] = @constraint(m2, [i=ID_BESS,j=J[1]],eb[i,j+1]-Ini_e_b[i]==Beffc[i]*pbc[i,j]-pbd[i,j]/Beffd[i])
   con7=m2.ext[:constraints][:con7] = @constraint(m2, [i=ID_BESS,j=J[1]],eb[i,j]==Ini_e_b[i])

   #Constraint charging-discharging batteries
   con8=m2.ext[:constraints][:con8] = @constraint(m2, [i=ID_BESS,j=J[1:end-1]],eb[i,j+1]-eb[i,j]==Beffc[i]*pbc[i,j]-pbd[i,j]/Beffd[i])

   #Constraint charging-discharging pump
   con6_Pump=m2.ext[:constraints][:con6_Pump] = @constraint(m2, [i=ID_Pump,j=J[end]],PEnd_e_b[i]-Pener[i,j]==Ppc[i,j]*PBeffc[i]-Ppd[i,j]/PBeffd[i])
   con7_Pump=m2.ext[:constraints][:con7_Pump] = @constraint(m2, [i=ID_Pump,j=J[1]],Pener[i,j]==PIni_e_b[i])
   con8_Pump=m2.ext[:constraints][:con8_Pump] = @constraint(m2, [i=ID_Pump,j=J[1:end-1]],Pener[i,j+1]-Pener[i,j]==Ppc[i,j]*PBeffc[i]-Ppd[i,j]/PBeffd[i])
   #


   ##Constraint Electrolyzer
   #Constraint  hydrogen production electrolyzer considering standby
   con9=m2.ext[:constraints][:con9] = @constraint(m2, [i=ID_E,j=J],hfe[i,j]==pe[i,j]/Eeff[i]-0.05*PEmax[i]/Eeff[i]*zestb[i,j]
   )

   #Constraint  hydrogen production electrolyzer without considering standby
   #con9=m2.ext[:constraints][:con9] = @constraint(m2, [i=ID_E,j=J],hfe[i,j]==pe[i,j]/Eeff[i])
   ...
   #Constraint end hydrogen value of the hydrogen storage
   con11=m2.ext[:constraints][:con11] = @constraint(m2, [i=ID_E,j=J[end]],End_h_s[i]==hss[i,j]+hfe[i,j]-hfgdinyec[i,j]/heffc[i]+hfgdcon[i,j]*heffd[i]-Eload_factor[i]*PEmax[i]/(Eeff[i]))
   #con11=m2.ext[:constraints][:con11] = @constraint(m2, [i=ID_E,j=J[end]],End_h_s[i]==hss[i,j])


   #Constraint initial value of the hydrogen storage
   con12=m2.ext[:constraints][:con12] = @constraint(m2, [i=ID_E,j=J[1]],hss[i,j+1]==Ini_h_s[i]+hfe[i,j]-hfgdinyec[i,j]/heffc[i]+hfgdcon[i,j]*heffd[i]-Eload_factor[i]*PEmax[i]/(Eeff[i]))
   #con12=m2.ext[:constraints][:con12] = @constraint(m2, [i=ID_E,j=J[1]],hss[i,j]==Ini_h_s[i])

   #Constraint charging-discharging of the hydrogen storage
   con13=m2.ext[:constraints][:con13] = @constraint(m2, [i=ID_E,j=J[1:end-1]],hss[i,j+1]==hss[i,j]+hfe[i,j]-hfgdinyec[i,j]/heffc[i]+hfgdcon[i,j]*heffd[i]-Eload_factor[i]*PEmax[i]/(Eeff[i]))

   #Bounds electrolyzer considering standby
   con13_1=m2.ext[:constraints][:con13_1] = @constraint(m2, [i=ID_E,j=J],pe[i,j].<=PEmax[i]*ze[i,j]+0.05*PEmax[i]*zestb[i,j])
   con13_2=m2.ext[:constraints][:con13_2] = @constraint(m2, [i=ID_E,j=J],pe[i,j].>=PEmin[i]*ze[i,j]+0.05*PEmax[i]*zestb[i,j])

   #Bounds electrolyzer without considering standby
   #con13_1=m2.ext[:constraints][:con13_1] = @constraint(m2, [i=ID_E,j=J],pe[i,j].<=PEmax[i]*ze[i,j])
   #con13_2=m2.ext[:constraints][:con13_2] = @constraint(m2, [i=ID_E,j=J],pe[i,j].>=PEmin[i]*ze[i,j])

   #Status constraint of electrolyzers
   con13_2_1=m2.ext[:constraints][:con13_2_1] = @constraint(m2, [i=ID_E,j=J[1]],zesu[i,j].>=ze[i,j]-1+(zestb[i,j]-0)) 

   #Startup constraint of electrolyzers
   con13_2_2=m2.ext[:constraints][:con13_2_2] = @constraint(m2, [i=ID_E,j=J[2:end]],zesu[i,j].>=(ze[i,j]-ze[i,j-1])+(zestb[i,j]-zestb[i,j-1])) 

   #Startup constraint of electrolyzers
   con13_2_3=m2.ext[:constraints][:con13_2_3] = @constraint(m2, [i=ID_E,j=J],zestb[i,j]+ze[i,j].<=1
   ) #Standby constraint of electrolyzers

   #con13_2_4=m2.ext[:constraints][:con13_2_4] = @constraint(m2, [i=ID_E,j=J], ze[i,j]==0)
   #Power consumption in standby=5% of nominal power according to chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://arxiv.org/pdf/2306.10962
   ...
   #Power consumption of the electrolyzer
   con13_8=m2.ext[:constraints][:con13_8] = @constraint(m2, [i=ID_E,j=J],pe_c[i,j]==compresor_power[i]*(hfe[i,j]+hfgdcon[i,j]+hfgdinyec[i,j]))


   #Constraint maximum frequency variation
   #Constraints frequency nadir as rotate second order cone
   con14_1=m2.ext[:constraints][:con14_1] = @constraint(m2,[i=ID,j=J],y[i,j] ==2*deltaf*(sum(Inertia_Expression[:,j])-Inertia_Expression[i,j])/FO-sum(re[:, j])*Dte/2-sum(rb[:, j])*Dtb/2)
   con14_2=m2.ext[:constraints][:con14_2] = @constraint(m2,[i=ID,j=J],z[i,j] ==(sum(rg[:, j])+sum(rgp[:,j])-rg[i, j])/Dtg)
   con14_3=m2.ext[:constraints][:con14_3] = @constraint(m2,[i=ID,j=J],[y[i,j]; z[i,j]; pl[j]-sum(re[:, j])-sum(rb[:,j])] in RotatedSecondOrderCone())


   con14_1_Pump=m2.ext[:constraints][:con14_1_Pump] = @constraint(m2,[i=ID_Pump,j=1],yp[i,j] ==2*deltaf*(sum(Inertia_Expression[:,j])-Inertia_Expression[i,j]*zpcommit[i,j])/FO-sum(re[:, j])*Dte/2-sum(rb[:, j])*Dtb/2)
   con14_2_Pump=m2.ext[:constraints][:con14_2_Pump] = @constraint(m2,[i=ID_Pump,j=1],zp[i,j] ==(sum(rg[:, j])+sum(rgp[:,j])-rgp[i, j])/Dtg)
   con14_3_Pump=m2.ext[:constraints][:con14_3_Pump] = @constraint(m2,[i=ID_Pump,j=1],[yp[i,j]; zp[i,j]; pl[j]-sum(re[:, j])-sum(rb[:,j])] in RotatedSecondOrderCone())

   #constraints nadir occurrence time
   con15= m2.ext[:constraints][:con15] = @constraint(m2, [i=ID,j=J],pl[j].>=sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j])+sum(rgp[:,j])-rg[i, j])*Dtb/Dtg)
   con16=m2.ext[:constraints][:con16] = @constraint(m2, [i=ID,j=J],pl[j].<=0.000001 + sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j])+sum(rgp[:,j])-rg[i, j]))

   con15_pump= m2.ext[:constraints][:con15_pump] = @constraint(m2, [i=ID_Pump,j=J],pl[j].>=sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j])+sum(rgp[:,j])-rgp[i,j])*Dtb/Dtg)
   con16_pump=m2.ext[:constraints][:con16_pump] = @constraint(m2, [i=ID_Pump,j=J],pl[j].<=0.000001 + sum(rb[:,j])+sum(re[:,j])+(sum(rg[:, j])+sum(rgp[:,j])-rgp[i,j]))
   #constraint ROCOF

   con17=m2.ext[:constraints][:con17] = @constraint(m2, [i=ID,j=J], pl[j]*FO/2 .<=rocofmax*(sum(Inertia_Expression[:,j])-Inertia_Expression[i,j]))
   con17_pump=m2.ext[:constraints][:con17_pump] = @constraint(m2, [i=ID_Pump,j=J], pl[j]*FO/2 .<=rocofmax*(sum(Inertia_Expression[:,j])-Inertia_Expression[i,j]))

   #QSS frequency constraint
   con18=m2.ext[:constraints][:con18] = @constraint(m2, [i=ID,j=J],pl[j].<=0.00001+sum(re[:, j])+sum(rb[:, j])+(sum(rg[:, j])+sum(rgp[:,j])-rg[i, j]))
   con18_pump=m2.ext[:constraints][:con18_pump] = @constraint(m2, [i=ID_Pump,j=J],pl[j].<=0.00001+sum(re[:, j])+sum(rb[:, j])+(sum(rg[:, j])))
...
#=
   con_rg_fix_1=m2.ext[:constraints][:con_rg_fix_1] = @constraint(m2, [i=ID[100:134],j=J],
   rg[i,j]==maxFrdelivarable[i]*zuc[i,j]
   )

   con_rg_fix_1_2=m2.ext[:constraints][:con_rg_fix_1_2] = @constraint(m2, [i=ID[1:100],j=J],
   rg[i,j]==maxFrdelivarable[i]*zuc[i,j]
   )
=#
   ...
   con_zuc_fix=m2.ext[:constraints][:con_zuc_fix] = @constraint(m2, [i=ID,j=J],
   zuc[i,j]==zucvaluesfix[i,j]
   )
   con_zuc_fix_Pump=m2.ext[:constraints][:con_zuc_fix_Pump] = @constraint(m2, [i=ID_Pump,j=J],
   zpcommit[i,j]==zpcommitvaluesfix[i,j]
   )

   con_ze_fix=m2.ext[:constraints][:con_ze_fix] = @constraint(m2, [i=ID_E,j=J],
   ze[i,j]==zevaluesfix[i,j]
   )
   con_zestb_fix=m2.ext[:constraints][:con_zestb_fix] = @constraint(m2, [i=ID_E,j=J],
   zestb[i,j]==zestbvaluesfix[i,j]
   )
   con_zb_fix=m2.ext[:constraints][:con_zb_fix] = @constraint(m2, [i=ID_BESS,j=J],
   zb[i,j]==zbvaluesfix[i,j]
   )
...
g = value.(m2.ext[:variables][:g])*Pbase
rg= value.(m2.ext[:variables][:rg])*Pbase
re= value.(m2.ext[:variables][:re])*Pbase
rb= value.(m2.ext[:variables][:rb])*Pbase
pl= value.(m2.ext[:variables][:pl])*Pbase
pbc= value.(m2.ext[:variables][:pbc])*Pbase
pbd= value.(m2.ext[:variables][:pbd])*Pbase
eb= value.(m2.ext[:variables][:eb])*Pbase
pe= value.(m2.ext[:variables][:pe])*Pbase
hfe= value.(m2.ext[:variables][:hfe])*Mbase
hfgdinyec= value.(m2.ext[:variables][:hfgdinyec])*Mbase
hfgdcon= value.(m2.ext[:variables][:hfgdcon])*Mbase
hss= value.(m2.ext[:variables][:hss])*Mbase
zucvalues=  value.(m2.ext[:variables][:zuc])
pe_c= value.(m2.ext[:variables][:pe_c])*Pbase
wvalues=  value.(m2.ext[:variables][:w])
vvalues=  value.(m2.ext[:variables][:v])
RCU= value.(m2.ext[:variables][:RCU])*Pbase
ze= value.(m2.ext[:variables][:ze])
zesu= value.(m2.ext[:variables][:zesu])
zestb= value.(m2.ext[:variables][:zestb])
zb= value.(m2.ext[:variables][:zb])
Ppd= value.(m2.ext[:variables][:Ppd])*Pbase
Ppc= value.(m2.ext[:variables][:Ppc])*Pbase
Pener= value.(m2.ext[:variables][:Pener])*Pbase
zpcommit= value.(m2.ext[:variables][:zpcommit])
rgp= value.(m2.ext[:variables][:rgp])*Pbase
...
operational_costs=objective_value(m2)
