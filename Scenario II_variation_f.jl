...
folder_name_plot="UC_CRE_$(RE_costs)_CRG_$(RG_costs)_IW_$(Installed_W_F)_IS_$(Installed_S_F)_HC_$(Costs_hydrogen)_delta_$(deltafreal)_S2_rocofmax_$(rocofmax_real)_variation_frequency"
...
fig_curt = Figure()
ax = Axis(fig_curt[1, 1];
    title = "Wind generation and renewable curtailment",
    xlabel = "Time [h]",
    ylabel = "Power [GW]"
)

lines!(ax, RCUvec ./ 1000;
    label = "RCU",
    linewidth = 2,
    color = :red
)
...
lines!(ax, Net_renewable;
    label = "Injected renewable",
    linewidth = 2,
    color = :orange
)
...
# Lines
lines!(ax, 1:number_hours, sum_reserve_g;
    label = "Reserve Generators",
    linewidth = 2,
    color = :red
)

lines!(ax, 1:number_hours, sum_reserve_b;
    label = "Reserve BESS",
    linewidth = 2,
    color = :green
)

lines!(ax, 1:number_hours, sum_reserve_e;
    label = "Reserve Electrolyzer",
    linewidth = 2,
    color = :blue
)

# Legend below the plot
fig[2, 1] = Legend(fig, ax; orientation = :horizontal, framevisible = false)

display(fig)

save_path = joinpath(folder_name_plot, "reserves.png")
save(save_path, fig)
display(fig)
...
ax = Axis(fig[1, 1];
    title = "Reserve and Loss of Power",
    xlabel = "Time [h]",
    ylabel = "Power [MW]"
)

lines!(ax, 1:number_hours, plvec;
    label = "Loss of power",
    linewidth = 2,
    color = :purple,
    linestyle = :solid
...
lines!(ax, 1:number_hours, total_reserve;
    label = "Total reserve",
    linewidth = 2,
    color = :red,
    linestyle = :dash
...
save_path = joinpath(folder_name_plot, "reserves_and_loss_of_power_plot.png")
save(save_path, fig)
...
scatter!(ax, x_Inertia, y_Inertia; markersize = 5)
...
scatter!(ax, x_Deltaf_nadir_rg_2, y_Deltaf_nadir_rg_2; markersize = 5, color = :green)
...
x_rocof = collect(keys(rocof_post_opt))
y_rocof = collect(values(rocof_post_opt))
...
scatter!(ax, x_rocof, y_rocof; markersize = 5, color = :red)
...
scatter!(ax, x_Inertia_energy, y_Inertia_energy; markersize = 5)
...
re=5000*0.5
ic_e=2.5
Initial_inertia = copy(Inertia_nadir_energy)
rg_ini=copy(sum_reserve_g)
...
if Initial_pl[j]<= re + sum(rbvec[:,j]) + (sum(rg_ini[j]) + rgpvector[j])*0.2*0.15
           Delta_f_electrolyzer_N_1 = FO_base*Initial_pl[j]^2 / (4 * Initial_inertia[j] *(((sum(rg_ini[j]) + rgpvector[j])/Dtg  +re / Dte +sum(rbvec[:,j]) / Dtb)) ) # Hz
           println("Delta f N-1 contingency: ", Delta_f_electrolyzer_N_1, " Hz")
           println("condition 1 holds")
        else
            Delta_f_electrolyzer_N_1 = FO_base / (4 * Initial_inertia[j]) * (
...
println("Delta f N-1 contingency without electrolyzer: ", FO_base/(4*Inertia_nadir_energy[j])*((plvec[j]-sum(revec[:,j])-sum(rbvec[:,j]))^2*Dtg/(sum(rgvec[:,j])+sum(rgpvector[j]))+sum(revec[:,j])*Dte+sum(rbvec[:,j])*Dtb), " Hz")
...
for i in ID
    ...
    if value(Inertia_Expression)[i,j] * Pbase > 0
        ...
        if Initial_pl_2[j] <= re_2 + sum(rbvec[:,j]) + (sum(rg_ini_2[j]) + rgpvector[j])*0.2*0.15
...
if Initial_pl[j]<= sum(rbvec[:,j]) + (sum(rg_ini[j]) + rgpvector[j])*0.2*0.15
...
for i in ID
    ...
    if value(Inertia_Expression)[i,j] * Pbase > 0
        ...
        if Initial_pl_3[j] <= sum(rbvec[:,j]) + (sum(rg_ini_3[j]) + rgpvector[j])*0.2*0.15
